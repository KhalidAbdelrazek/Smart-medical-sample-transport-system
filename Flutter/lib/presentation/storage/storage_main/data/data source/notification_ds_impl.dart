...
class NotificationDsImpl extends NotificationDs {
  ...
  Future<Either<Failures, ReturnedCarsResponseDm>> getReturnedCars() async {
    ...
    try {
      ...
      if (!connectivityResult.contains(ConnectivityResult.none)) {
        ...
        ReturnedCarsResponseDm returnedCarsResponseDm =
            ReturnedCarsResponseDm.fromJson(response.data);
        ...
        return Left(
          ServerError(
            errorMessage:
                returnedCarsResponseDm.message?.toString() ?? "Server Error",
          ),
        );
      } else {
  ...
  @override
  Future<Either<Failures, ConfirmCarReturnDm>> confirmReturnedCar(
    int carId,
  ) async {
    final List<ConnectivityResult> connectivityResult = await Connectivity()
        .checkConnectivity();
    ...
    try {
      ...
      if (!connectivityResult.contains(ConnectivityResult.none)) {
        ...
        // Map the entire response (success, message, data, errors)
        ConfirmCarReturnDm confirmCarReturnDm = ConfirmCarReturnDm.fromJson(
          response.data,
        );
        ...
        return Left(
          ServerError(
            errorMessage:
                confirmCarReturnDm.message?.toString() ?? "Server Error",
          ),
        );
      } else {
        return Left(NetworkError(errorMessage: 'extra.no_internet_lower'.tr()));
