...
Business logic for transport operations (storage employee actions).

Key functions:
    add_sample_to_car   — Assigns a sample to a car, updates statuses
    dispatch_car        — Dispatches all samples in a car for delivery
"""
...
def add_sample_to_car(sample_id, car_id):
    """
    ...
    Returns the updated TransportRequest.
    """
    try:
    ...
    if sample.status != 'REQUESTED':
        raise ValidationError(
            f"Sample must have status REQUESTED to be added to a car. "
            f"Current status: {sample.status}"
        )
...
def dispatch_car(car_id):
    ...
    Rules:
    - Car must exist
    - Car must have at least one LOADED transport request (cannot dispatch empty car)
    ...
    Returns the list of dispatched transport requests.
    This operation is wrapped in a transaction for data integrity.
    """
    try:
        car = Car.objects.get(id=car_id)
    except Car.DoesNotExist:
    ...
    if not loaded_requests.exists():
        raise ValidationError(
            "Cannot dispatch an empty car. "
            "Please add at least one sample before dispatching."
        )
