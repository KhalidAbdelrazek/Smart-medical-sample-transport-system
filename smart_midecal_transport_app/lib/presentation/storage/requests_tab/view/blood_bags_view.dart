...
import '../widgets/blood_bag_card.dart';
import '../widgets/section_header.dart';

/// Blood Bags sub-tab view
class BloodBagsView extends StatelessWidget {
  const BloodBagsView({super.key});

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);

    return BlocBuilder<BloodBagsCubit, BloodBagsState>(
      builder: (context, state) {
        return RefreshIndicator(
          onRefresh: () => context.read<BloodBagsCubit>().refresh(),
  ...
  Widget _buildContent(
    ...
    BloodBagsState state,
    ThemeData theme,
  ) {
    if (state is BloodBagsLoading || state is BloodBagsInitial) {
      ...
      return Center(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16.h),
            ElevatedButton(
              onPressed: () => context.read<BloodBagsCubit>().loadRequests(),
    ...
    if (state is BloodBagsLoaded) {
      final cubit = context.read<BloodBagsCubit>();
